...
def write_json_solution_mip_flexi(gp_model, var_x, data: InputData, allTasks, distances: List[List[int]], filepath: str):
    """
                ...
                for element in tuple: 
                    if element == startelem:
                        ...
                        if id != "PE-FWI-5-5985":  # Exclude specific ID -> Depot
                            start_time += distances[startelem][element]
                            route_list.append({
                            ...
                            startelem = element
                            start_time += allTasks[element].service_time
    ...
    for day in range(data.days): 
        day_list = []
        for c in range(day_start,data.cohort_no * (day +1)):
            day_list.append(cohort_lists[c])

        day_start = data.cohort_no * (day +1)
...
def write_txt_solution(gp_model, var_x, data: InputData, allTasks: List[Union[OptionalTask, MainTask]], file_path: str) -> None:
    """
    Writes optimization gap, runtime, number of constraints, and number of variables 
    from a solved Gurobi model into a text file.
