...
class ImprovementAlgorithm:
    ...
    def __init__(self, inputData:InputData, neighborhoodEvaluationStrategy:str = 'BestImprovement', neighborhoodTypes:list[str] = ['Swap'], no_lots_boolean:bool = False):
    ...
    def CreateNeighborhood(self, neighborhoodType:str, bestCurrentSolution:Solution) -> BaseNeighborhood:
        ...
        if neighborhoodType == 'Swap':
            return SwapNeighborhood(self.InputData, bestCurrentSolution.Permutation,bestCurrentSolution.lot_matrix, self.EvaluationLogic, self.SolutionPool, self.no_lots)
        elif neighborhoodType == 'Insertion':
            return InsertionNeighborhood(self.InputData, bestCurrentSolution.Permutation,bestCurrentSolution.lot_matrix, self.EvaluationLogic, self.SolutionPool,self.no_lots)
        elif neighborhoodType == 'BlockK3':
            return BlockNeighborhoodK3(self.InputData, bestCurrentSolution.Permutation,bestCurrentSolution.lot_matrix, self.EvaluationLogic, self.SolutionPool,self.no_lots)
        elif neighborhoodType == 'TwoEdgeExchange':
            return TwoEdgeExchangeNeighborhood(self.InputData, bestCurrentSolution.Permutation,bestCurrentSolution.lot_matrix, self.EvaluationLogic, self.SolutionPool,self.no_lots)
        elif neighborhoodType == 'LotSwap':
            return SwapLotNeighborhood(self.InputData, bestCurrentSolution.Permutation, bestCurrentSolution.lot_matrix,self.EvaluationLogic, self.SolutionPool,self.no_lots)
        elif neighborhoodType == 'LotInsertion':
            return InsertionLotNeighborhood(self.InputData, bestCurrentSolution.Permutation, bestCurrentSolution.lot_matrix,self.EvaluationLogic, self.SolutionPool,self.no_lots)
        elif neighborhoodType == 'SplitMutation':
            return SplitMutationNeighborhood(self.InputData, bestCurrentSolution.Permutation, bestCurrentSolution.lot_matrix,self.EvaluationLogic, self.SolutionPool,self.RNG,self.no_lots)
        else:
...
class IterativeImprovement(ImprovementAlgorithm):
    ...
    def __init__(self,  inputData:InputData, neighborhoodEvaluationStrategy:str = 'BestImprovement', neighborhoodTypes:list[str] = ['Swap'], no_lots_boolean:bool = False):
        super().__init__(inputData, neighborhoodEvaluationStrategy, neighborhoodTypes, no_lots_boolean)
...
class IteratedGreedy(ImprovementAlgorithm):
    ...
    def Construction(self, removedJobs:list[int], partial_permutation:list[int], partial_lotmatrix:list[list[int]]) -> Solution:
        ...
        completeSolution = Solution(self.InputData.InputJobs, self.InputData.InputStages, partial_permutation, partial_lotmatrix, self.no_lots)
